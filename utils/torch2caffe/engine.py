...
class TransLog(object):
    ...
    def add_layer(self, name='layer'):
        ...
        name = '{}{}{}'.format("torch_", name, self.detail_layers[name])
...
def _pool(type, raw, input, x, kernel_size, stride, padding, ceil_mode):
    ...
    if ceil_mode == False and stride is not None:
        ...
        print(oheight, owidth, input.size(), _pair(kernel_size), _pair(padding), _pair(stride), dir(raw))
        if (oheight != 0 or owidth != 0) and raw.__name__ != 'adaptive_avg_pool2d':
...
def _view(input, *args):
    ...
    if not NET_INITTED or id(input) not in log._blobs.data.keys():
        return x
...
def _add(input, *args):
    ...
    if not NET_INITTED or id(input) not in log._blobs.data.keys() or id(args[0]) not in log._blobs.data.keys():
        return x
...
def _addcmul(raw, *args):
    x = raw(*args)
    if id(args[2]) not in log._blobs.data.keys() or id(args[0]) not in log._blobs.data.keys() or id(
            args[3]) not in log._blobs.data.keys():
        return x
    layer_name = log.add_layer(name='axpy')
    top_blobs = log.add_blobs([x], name='axpy_blob')
    layer = caffe_net.Layer_param(name=layer_name, type='Axpy',
                                  bottom=[log.blobs(args[2]),
                                          log.blobs(args[3]),
                                          log.blobs(args[0])],
                                  top=top_blobs)
    log.cnet.add_layer(layer)
...
def _bmm(raw, *args):
    x = raw(*args)
    if id(args[0]) not in log._blobs.data.keys() or id(args[1]) not in log._blobs.data.keys():
        return x
    layer_name = log.add_layer(name='matrixmultiplication')
    top_blobs = log.add_blobs([x], name='matrixmultiplication_blob')
    layer = caffe_net.Layer_param(name=layer_name, type='MatrixMultiplication',
...
torch.addcmul = Rp(torch.addcmul, _addcmul)
torch.bmm = Rp(torch.bmm, _bmm)

for t in [torch.Tensor]:
    raw_view = t.view
    t.view = _view
    # t.view = [raw_view, _view]
    raw_mean = t.mean
    t.mean = _mean
    raw__add__ = t.__add__
    # t.__add__ = [raw__add__, _add]
    t.__add__ = _add
...
def trans_net(net, input_var, name='TransferedPytorchModel'):
    ...
    # out = net.forward(input_var, key_ = "")
    print('Transform Completed')


def trans_net_key(net, input_var, name, key):
    print('Starting Transform, This will take a while')
    log.init([input_var])
    log.cnet.net.name = name
    log.cnet.net.input.extend([log.blobs(input_var)])
